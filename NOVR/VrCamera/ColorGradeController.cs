...
public class ColorGradeController : MonoBehaviour
{
    ...
    private void HandleGammaShortcuts()
    {
        ...
        var steps = 0;
        if (Input.GetKeyDown(config.ColorGammaIncreaseShortcut.Value)) steps++;
        if (Input.GetKeyDown(config.ColorGammaDecreaseShortcut.Value)) steps--;
        if (steps == 0)
        {
            return;
        }
        ...
        var stepped = Mathf.Round(config.ColorGamma.Value / GammaStep) + steps;
        var value = Mathf.Clamp(stepped * GammaStep, GammaMin, GammaMax);
        if (Mathf.Approximately(value, config.ColorGamma.Value))
        {
    ...
    private void ReportIfDue()
    {
        ...
        var value = ModConfiguration.Instance.ColorGamma.Value;
    ...
    private void Apply()
    {
        ...
        var gamma = ModConfiguration.Instance.ColorGamma.Value;
