...
public static class StickCursorConfig
{
    ...
    public static void Bind(ConfigFile config)
    {
        StickCursor = config.Bind(
            ...
            "Stick Cursor",
            true,
            "Move the VR cursor with the game's own view axes ('Pan View'/'Tilt View' — normally a thumbstick or hat), so menus and the map can be used without a mouse or a tracked controller. Unlike the head-gaze cursor it does not follow your head: it stays where you left it, it is unaffected by recentering, and you can look somewhere else while aiming it. The mouse still works — moving it takes the cursor back at once, and so does a motion controller picked up, for as long as it is held. With both controllers up, the one that pulled its trigger last is the one pointing — pull the other's trigger and the laser moves to it — or set 'Cursor Input Source' to a hand to fix it there. On by default; while it is on it overrides both 'Head Gaze Cursor' and 'Cursor Input Source', so turn it off to fall back to whichever of those two is selected.");
